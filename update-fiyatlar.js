...
// --- BURASI DOĞRULANMADI ---
// API anahtarınızı aldıktan sonra tarayıcıda bir kez şu adresi deneyin:
//   https://ucuzyakitbul.com.tr/api/stations?city=Ankara
// Dönen JSON'daki gerçek alan adlarına (il/district/city, fuelType/yakitTuru,
// price/fiyat gibi) bakıp aşağıdaki üç satırı gerekirse güncelleyin.
async function fetchIlIstasyonlari(il) {
  const url = BASE_URL + "/stations?city=" + encodeURIComponent(il) + "&limit=500";
  const data = await fetchJSON(url);
  return data.stations || data.data || data.results || [];
}
...
function groupByDistrict(il, stations) {
  ...
  for (const s of stations) {
    ...
    const fuel = String(s.fuelType || s.yakitTuru || s.fuel_type || "").toLowerCase();
    const price = Number(s.price ?? s.fiyat);
    if (!price) continue;
    if (fuel.includes("benzin") || fuel.includes("gasoline")) bucket.benzin.push(price);
    else if (fuel.includes("motorin") || fuel.includes("dizel") || fuel.includes("diesel")) bucket.motorin.push(price);
    else if (fuel.includes("lpg") || fuel.includes("otogaz")) bucket.lpg.push(price);
  }
